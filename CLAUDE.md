...
Dois cron jobs (todos no mesmo workflow `analista.yml`):
- **a cada 2 horas** (`0 */2 * * *`) → `python main.py analyze`
...
```json
{
  ...
  "pick_btts": false, "hit_btts": false
}
```
Nota: campos `po`/`pb`/`hw`/`dr`/`aw` em escala 0-100 (não 0-1).
